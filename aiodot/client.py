...
class MyDotClient:
    ...
    async def upload_header(self, file_path: str, filename: str = "header.jpg", content_type: str = "image/jpeg") -> Optional[str]:
        import os
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"File not found: {file_path}")
        req = await self._post("/auth/profile/header/upload/", json={
            "filename": filename,
            "content_type": content_type,
        })
        upload_url = req.get("upload_url") or req.get("url")
        ...
        async with self.http.put(upload_url, data=data, headers={"Content-Type": content_type}) as resp:
            if resp.status not in (200, 201, 204):
                return None
        return req.get("header_key") or req.get("key")
    ...
    async def get_conversations(self, tab: str = "all", offset: int = 0, limit: int = 20) -> Dict[str, Any]:
        params = {"tab": tab, "offset": offset, "limit": limit}
        return await self._get("/chat/conversations/", params=params)

    async def get_conversation(self, conversation_id: str) -> Dict[str, Any]:
        return await self._get(f"/chat/conversations/{conversation_id}/")

    async def send_message(self, conversation_id: str, content: str) -> Dict[str, Any]:
        return await self._post(f"/chat/conversations/{conversation_id}/messages/", json={"content": content})

    async def get_messages(self, conversation_id: str, offset: int = 0, limit: int = 50) -> Dict[str, Any]:
        params = {"offset": offset, "limit": limit}
        return await self._get(f"/chat/conversations/{conversation_id}/messages/", params=params)

    async def mark_conversation_read(self, conversation_id: str) -> bool:
        await self._post(f"/chat/conversations/{conversation_id}/read/")
        return True

    async def get_unread_count(self) -> int:
        data = await self._get("/chat/unread-count/")
        return data.get("count", 0)

    async def get_connection_status(self) -> Dict[str, Any]:
        return await self._get("/chat/connection-status/")

    async def search_conversations(self, query: str, tab: str = "all", offset: int = 0) -> Dict[str, Any]:
        params = {"q": query, "tab": tab, "offset": offset}
        return await self._get("/chat/search/", params=params)

    async def get_donators(self, post_id: str, limit: int = 20, cursor: str = None) -> Dict[str, Any]:
        params = {"limit": limit}
        if cursor:
            params["cursor"] = cursor
        return await self._get(f"/posts/{post_id}/donators/", params=params)

    async def get_great_donators(self, limit: int = 20) -> List[Dict[str, Any]]:
        params = {"limit": limit}
        data = await self._get("/reward/most-donators/", params=params)
        return data.get("results", []) if isinstance(data, dict) else data
